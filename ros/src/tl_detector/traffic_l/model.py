...
#     print np.min(img_hsv),np.max(img_hsv)
    ...
    rate = np.count_nonzero(mask)*1.0 / (desired_dim[0] * desired_dim[1])*1.0
    print np.count_nonzero(mask), desired_dim[0], desired_dim[1], rate, Threshold
    if rate > Threshold:
        return True
    else:
        return False
...
def detect_traffic_lights(PATH_TO_TEST_IMAGES_DIR, MODEL_NAME, Num_images, plot_flag=False,transfer_seesion=True):
    """
...
#     PATH_TO_CKPT = MODEL_NAME + '/frozen_inference_graph.pb'
    ...
    TEMP_MODEL_NAME = ""
    for i in MODEL_NAME:
        TEMP_MODEL_NAME = TEMP_MODEL_NAME+i
    PATH_TO_CKPT = TEMP_MODEL_NAME + '/frozen_inference_graph.pb'

    # List of the strings that is used to add correct label for each box.
    PATH_TO_LABELS = 'traffic_l/mscoco_label_map.pbtxt'
    ...
    #--------Download model----------
    if path.isfile(MODEL_FILE) is False:
        print("Downloading Model")
        opener = urllib.request.URLopener()
        opener.retrieve(DOWNLOAD_BASE + MODEL_FILE, MODEL_FILE)
        untar_file(MODEL_FILE)
    if path.isdir(TEMP_MODEL_NAME) is False:
